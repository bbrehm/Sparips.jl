...
function sparsify_ctree(ct::Ctree{dist_T, point_T, f_T}, eps_rel::Real, Nmax=0; strategy=APriori()) where {f_T,point_T,dist_T}
...
function sparse_edges_aposteriori(ct::Ctree{dist_T, point_T, f_T}, prec, Nmax=0) where {dist_T, point_T, f_T}
    if Nmax <= 0 || Nmax >= length(ct.data)
        Nmax = length(ct.data)
        eps_abs = f_T(0)
    else
